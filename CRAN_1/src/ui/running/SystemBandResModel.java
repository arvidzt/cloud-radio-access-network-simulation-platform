/*
 * 类名：SystemBandResModel
 * 功能：run1的频带划分模块中对应数据库中【系统频带资源】表格模型类
 */
package ui.running;
...
public class SystemBandResModel extends AbstractTableModel{
	//rowData用来存放行数据 
			//columNames用来存放列名
			Vector rowData,columNames;
			Vector hang;
			int search_rownum;
			public SystemBandResModel()
			{
				...
				BBU_sql sqlh=null;
					try {
						...
						while(rs.next())
						{
							...
							hang.add(rs.getString(4));//AllRes
							hang.add(rs.getString(5));//BandNum
							hang.add(rs.getString(6));//BandName
							hang.add(rs.getString(7));//PublicBand
							hang.add(rs.getString(8));//BandCenter
							hang.add(rs.getString(9));//BandWidth
							hang.add(rs.getString(10));//BbuPoolId
							hang.add(rs.getString(11));//BbuId
							//加入到rowData
							rowData.add(hang);
