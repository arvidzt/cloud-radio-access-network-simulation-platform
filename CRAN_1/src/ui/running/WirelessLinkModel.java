...
public class WirelessLinkModel extends AbstractTableModel{
	//rowData用来存放行数据 
	//columNames用来存放列名
		...
		BBU_sql sqlh=null;
			try {
				...
				while(rs.next())
				{
					...
					hang.add(rs.getString(4));//Circumstance
					hang.add(rs.getString(5));//FrequencyRage
					hang.add(rs.getString(6));//K1
					hang.add(rs.getString(7));//K2
					//加入到rowData
					rowData.add(hang);
